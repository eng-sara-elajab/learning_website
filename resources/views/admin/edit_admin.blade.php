...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Admin Panel -<span style="opacity: 0.8; margin-left: 10px">{{$admin->id == Auth::guard('admin')->user()->id ? 'Edit Profile' : 'Edit Admin'}}</span></h1>
                </div>
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
    ...
    <section class="content">
        <div class="container-fluid row">
            <div class="col-md-12 col-xs-12">
                <h5 style="color: darkorange; font-weight: bold; text-align: center; margin-top: 1%">Edit {{$admin->name}} information</h5>
            </div>
            <form class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 row" method="post" action="{{route('admin.update_admin', $admin->id)}}" enctype="multipart/form-data" style="margin-top: 3%">
                @csrf
                {{ method_field('PUT') }}
                <div class="col-xl-6 offset-xl-1 col-lg-6 offset-lg-1 col-md-6 offset-md-1 col-sm-12 col-12 form-group" style="border: solid 1px lightgrey; border-right: none; text-align: left; border-radius: 15px; opacity: 0.9">
                    <label style="margin-top: 10px; margin-left: 5px">Admin name</label>
                ...
                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12 form-group" style="border: solid 1px lightgrey; border-left: none; text-align: center; border-radius: 15px; opacity: 0.9">
                    ...
                    @else
                        <img src="/images/profiles/{{Auth::guard('admin')->user()->profile_photo}}" id="blah" style="width: 100%; height: 300px; margin-top: 10px; margin-bottom: 10px">
                    @endif
            ...
            @if($admin->id !== 1)
                <div class="col-xl-4 offset-xl-4 col-lg-4 offset-lg-4 col-md-4 offset-md-4 col-sm-4 offset-sm-4 col-4 offset-4">
                    <a href="{{route('admin.destroy_admin', $admin->id)}}" class="btn btn-danger btn-block">Delete {{$admin->name}}</a>
                </div>
            @endif
        </div>
    </section>
